...
{
    t_eReturnState Ret_e = RC_OK;
    static t_bool s_IsAdcCfgSet_b[FMKCDA_ADC_NB];
    ...
    if(Ret_e == RC_OK)
    {
        // Update information depending on wtehter the Adc has already been configured by a precedent call
        switch(s_IsAdcCfgSet_b[f_Adc_e])
        {
            case False:
            {
                s_IsAdcCfgSet_b[f_Adc_e] = (t_bool)True;
                g_AdcInfo_as[f_Adc_e].HwCfg_e = f_adcCfg_e;
            ...
            case True:
            {
                // verify cfg -> once on Adc config is set for a channel the other channel must have the same cfg
                if(g_AdcInfo_as[f_Adc_e].HwCfg_e != f_adcCfg_e)
                {
...
/*********************************
* s_FMKCDA_PreOperational
*********************************/
static t_eReturnState s_FMKCDA_PreOperational(void)
{
    t_eReturnState Ret_e = RC_OK;
    t_uint8 LLI_u8 = 0;
    static t_uint8 s_timerCnt_u8 = 0; 
    static t_uint8 s_chnlCnt_u8 = 0; 
    static t_bool s_IsAdcCfgDone_b = (t_bool)False;

    // Initialisation des ADC
    if(s_IsAdcCfgDone_b == (t_bool)False)
    {
        for(LLI_u8 = (t_uint8)0 ; (LLI_u8 < (t_uint8)FMKCDA_ADC_NB && Ret_e == RC_OK) ; LLI_u8++)
        {
            Ret_e = s_FMKCDA_Set_BspAdcCfg((t_eFMKCDA_Adc)LLI_u8,
                                            g_AdcInfo_as[LLI_u8].HwCfg_e);
            if(Ret_e == RC_OK)
            {
                // Initiasation du DMA pour l'instant on considère que tout est SCan DMA
                #warning "Only Scan/DMA mode is trreated for AdcChannel Configuration"
                Ret_e = FMKMAC_RqstDmaInit(FMKMAC_DMA_RQSTYPE_ADC1, (void *)&g_AdcInfo_as[LLI_u8].BspInit_s);
            }
        }
        
        if(LLI_u8 == FMKCDA_ADC_NB && Ret_e == RC_OK)
        {
            s_IsAdcCfgDone_b = (t_bool)True;
        }

    }
    // Initialisation des canaux ADC
    else
    {
        // Configurer 5 canaux par appel
        for(; s_chnlCnt_u8 < (t_uint8)(s_chnlCnt_u8 + FMKCDA_CHNLCFG_PER_CYCLE); s_chnlCnt_u8++)
        {
            if(s_timerCnt_u8 < FMKCDA_ADC_NB && 
               s_chnlCnt_u8 < FMKCDA_ADC_CHANNEL_NB &&
               g_AdcInfo_as[s_timerCnt_u8].Channel_as[s_chnlCnt_u8].IsChnlUsed_b == (t_bool)True)
            {
                Ret_e = s_FMKCDA_Set_BspChannelCfg((t_eFMKCDA_Adc)s_timerCnt_u8,
                                                    (t_eFMKCDA_AdcChannel)s_chnlCnt_u8);
                if(Ret_e != RC_OK)
                {
                    break;  // Arrêter la configuration si une erreur se produit
                }
            }
            
            // Passer au prochain ADC si tous les canaux de l'ADC actuel sont configurés
            if(s_chnlCnt_u8 >= FMKCDA_ADC_CHANNEL_NB - 1)
            {
                s_chnlCnt_u8 = 0; // Réinitialiser le compteur des canaux
                s_timerCnt_u8++;  // Passer au prochain ADC
                
                if(s_timerCnt_u8 >= FMKCDA_ADC_NB)
                {
                    break;  // Arrêter si tous les ADC ont été configurés
                }
            }
        }
        
        // Si tous les canaux de tous les ADC sont configurés, changer d'état
        if(s_timerCnt_u8 >= FMKCDA_ADC_NB && s_chnlCnt_u8 >= FMKCDA_ADC_CHANNEL_NB)
        {
            g_state_e = STATE_CYCLIC_OPE;
        }
    }
    return Ret_e;
}
...
static t_eReturnState s_FMKCDA_Set_BspChannelCfg(t_eFMKCDA_Adc f_Adc_e, t_eFMKCDA_AdcChannel f_channel_e)
{
    ...
    static t_uint8 counterRank_u8 = 0;
    ...
    if(Ret_e == RC_OK)
    {
        ...
        if(Ret_e == RC_OK)
        {
            ...
            BspChannelInit_s.Rank         = counterRank_u8;
            ...
            {
                // update mapping
                g_AdcBuffer_as[f_Adc_e].BspChnlmapp[f_channel_e] = counterRank_u8;
                g_AdcInfo_as[f_Adc_e].Channel_as[f_channel_e].IsChnlConfigured_b = (t_bool)True;
                counterRank_u8 += (t_uint8)1;
            }
            else
            {
                Ret_e = RC_ERROR_WRONG_STATE;
